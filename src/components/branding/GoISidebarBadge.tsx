...
export function GoISidebarBadge({ subtitle, description, showDescriptionOnMobile }: GoISidebarBadgeProps) {
  return (
    <div className="sidebar-brand-goi grid w-full justify-items-center gap-2">
      <div className="flex h-[112px] w-[112px] shrink-0 items-center justify-center overflow-hidden rounded-full bg-neutral-950 ring-2 ring-goi-gold-dim/30 shadow-[0_8px_28px_rgba(0,0,0,0.55)] max-md:h-[88px] max-md:w-[88px] light:bg-white light:shadow-[0_8px_28px_rgba(0,0,0,0.12)]">
        <img
          src={LOGO_SRC}
      ...
      <div className="sidebar-user w-full text-center text-sm text-neutral-400 light:text-zinc-600">{subtitle}</div>
      {description && (
        <p
          className={`mt-3 max-w-[20rem] text-center text-sm leading-relaxed text-neutral-500 light:text-zinc-600 ${showDescriptionOnMobile ? "" : "hidden md:block"}`}
        >
          {description}
        </p>
      )}
