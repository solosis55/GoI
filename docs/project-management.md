...
69. **Vercel (API serverless):** **`vercel.json`** (`installCommand`, `buildCommand` → **`npm run vercel-build`**, `outputDirectory` `dist`, rewrite `/api/*` → funcion **`api/index.mjs`** que importa **`server/dist/app.js`**); **`includeFiles`** incluye `server/dist/**` y **`server/data/store.json`**. En **`store.ts`**, si **`VERCEL`**, el JSON de trabajo va a **`/tmp/fitsocial-store.json`** con copia inicial desde el seed del repo; variable opcional **`FITSOCIAL_STORE_PATH`**. **`app.ts`** aplica **`trust proxy`** en produccion o Vercel. Documentacion en **`docs/deploy.md`** y **`server/.env.example`**.
